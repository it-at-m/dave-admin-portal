<template>
  ...
  <v-overlay color="white" v-else>
    <p style="text-align: center; color: black; font-weight: bold; font-size: xxx-large">
...
export default class App extends Vue {
  ...
  private static readonly URL_HANDBUCH_LINK: string = 'https://wilma.muenchen.de/web/senders/af10dc2a-8da5-4d24-815a-b6a9df4c686b/documents/54ddf065-d01f-4965-9bdf-c66ea47927c8';
  ...
  get isFachadmin() {
    if (BaseUrlProvider.isDevelopment() && this.$store.getters["user/hasNoAuthorities"]) {
      return true;
    } else {
      ...
      SucheService.getSuggestions(query).then((suggestions: SucheComplexSuggestsDTO) => {
        ...
        suggestions.wordSuggests.forEach((word: SucheWordSuggestDTO) => {
          this.suggestions.push(
              new Suggest(word.text, this.suggestionTypeVorschlag, "", "")
          );
        });

        suggestions.zaehlungenSuggests.forEach((zaehlung: SucheZaehlungSuggestDTO) => {
          this.suggestions.push(
              new Suggest(zaehlung.text, this.suggestionTypeZaehlung, zaehlung.zaehlstelleId, zaehlung.id)
          );
        });

        suggestions.zaehlstellenSuggests.forEach((zaehlstelle: SucheZaehlstelleSuggestDTO) => {
          this.suggestions.push(
              new Suggest(zaehlstelle.text, this.suggestionTypeZaehlstelle, zaehlstelle.id, "")
          );
        });
  ...
  private navigateToOpenZaehlungen() {
