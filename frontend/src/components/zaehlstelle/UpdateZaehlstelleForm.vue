    ...
    <v-row>
      <v-col cols="12" md="6">
        <v-form ref="form">
          <v-row no-gutters>
            <v-col cols="12" md="6">
              <span class="caption">Zählstellennummer</span><br/>
              <span class="text text-info">{{ zaehlstelle.nummer }}</span><br/>
            </v-col>
          </v-row>
          <v-row no-gutters>
            <v-col cols="12" md="12">
              <span class="caption">Stadtbezirk</span><br/>
              <span class="text text-info">{{ getStadtbezirksnummer }}</span><br/>
            </v-col>
          </v-row>
          <v-row no-gutters>
            <v-col cols="12" md="12">
              <span class="caption">Stadtbezirksviertel</span><br/>
              <span class="text text-info">{{ getStadtbezirksviertel }}</span><br/>
            </v-col>
          </v-row>
          <v-row>
            <v-col cols="12" md="12">
          ...
          <v-row no-gutters>
            <v-col cols="12" md="12">
              <v-checkbox v-model="zaehlstelle.sichtbarDatenportal"
                          color="grey darken-1"
                          hide-details
                          dense
              >
...
export default class UpdateZaehlstelleForm extends Vue {
  ...
  get getStadtbezirksnummer(): String {
    const stadtbezirksnummer: String | undefined = StadtbezirkToBeschreibung.get(this.zaehlstelle.stadtbezirkNummer)
  ...
  get getStadtbezirksviertel(): String {
    const stadtbezirksviertelMap: Map<number, string> | undefined = StadtbezirksviertelToBeschreibung.get(this.zaehlstelle.stadtbezirkNummer);
    if (stadtbezirksviertelMap != undefined) {
      const stadtbezirksviertelnummer: String | undefined = stadtbezirksviertelMap.get(this.getStadtbezirksviertelNummer);
  ...
  private addSuchwortToList() {
    ...
    if (!this.zaehlstelle.customSuchwoerter.includes(this.newSuchwort)) {
      this.zaehlstelle.customSuchwoerter.push(...this.newSuchwort.split(","));
    }
    this.newSuchwort = "";
  }

  private save(): void {
    ZaehlstellenService.saveZaehlstelle(this.zaehlstelle).then(() => {
      this.$emit('saved');
    }).catch(error => {
