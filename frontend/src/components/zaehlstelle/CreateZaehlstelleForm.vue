          ...
          <v-row no-gutters>
            <v-col cols="12" md="6">
              <div>
                ...
                <span class="text text-info">{{ zaehlstellenummer }}</span><br/><br/>
              </div>
            </v-col>
          </v-row>
          <v-row no-gutters>
            <v-col cols="12" md="12">
              <v-autocomplete
                  ...
                  dense
                  label="Stadtbezirk"
                  :rules="[() => !!zaehlstelle.stadtbezirkNummer || pflichtfeldText]"
                  required
              ></v-autocomplete>
            </v-col>
          </v-row>
...
export default class ZaehlstelleForm extends Vue {
  ...
  private validZaehlstelle: boolean = false;

  private zaehlstelle: ZaehlstelleDTO = DefaultObjectCreator.createDefaultZaehlstelleDTO();

  private stadtbezirksviertelModel: string = '';
  private laufendeNummer: string = '';
  ...
  get getStadtbezirksviertel(): Array<KeyVal> {
    return stadtbezirksviertel(this.zaehlstelle.stadtbezirkNummer ? this.zaehlstelle.stadtbezirkNummer.toString() : '');
  }
  ...
  createZaehlstellennummer(stadtbezirksviertel: KeyVal): void {
    if (stadtbezirksviertel && stadtbezirksviertel.value) {
      ...
      const idStartsWith: string = `${this.zaehlstelle.stadtbezirkNummer}${this.stadtbezirksviertelModel}`;
      ZaehlstellenService.getNextZaehlstellennummer(idStartsWith, this.zaehlstelle.stadtbezirkNummer)
  ...
  private save(): void {
    if (this.validZaehlstelle) {
      ...
      ZaehlstellenService.saveZaehlstelle(this.zaehlstelle).then(backendIdDTO => {
      ...
      this.$store.dispatch('snackbar/showToast', {
        ...
        snackbarTextPart1: 'Es wurde nicht alle Pflichtfelder ausgefüllt.'
      });
    }
  }
