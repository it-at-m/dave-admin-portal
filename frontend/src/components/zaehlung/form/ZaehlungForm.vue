  ...
  >
    ...
    >
      ...
      <v-tab-item ref="knotenUndLage">
        <knoten-richtung-form :height="SHEETHEIGHT" :zaehlstelle="zaehlstelle"/>
      </v-tab-item>
      <v-tab-item ref="fahrbeziehungen">
        <fahrbeziehung-kreisverkehr-form :height="SHEETHEIGHT" v-if="isKreisverkehr"/>
        <fahrbeziehung-form :height="SHEETHEIGHT" v-else/>
      </v-tab-item>
...
export default class ZaehlungForm extends Vue {
  ...
  private save(): void {
    ...
    ZaehlungService.saveZaehlung(copy, this.zaehlstelle.id).then(() => {
      this.$emit('saved');
