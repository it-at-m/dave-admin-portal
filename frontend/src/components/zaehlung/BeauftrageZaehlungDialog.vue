...
export default class BeauftrageZaehlungDialog extends Vue {
  ...
  get dialogtitle(): string {
    return this.isBeauftragen ? 'Dienstleister beauftragen' : 'Dienstleister korrigieren';
  }

  get dialogicon(): string {
    return this.isBeauftragen ? 'mdi-account-arrow-right' : 'mdi-account-convert-outline';
  }

  private loadAllActiveDienstleister() {
    this.dienstleister = [];
    this.dienstleisterIsLoading = true;
    DienstleisterService.getAllActive().then((dienstleisterDTOS:Array<DienstleisterDTO>) => {
