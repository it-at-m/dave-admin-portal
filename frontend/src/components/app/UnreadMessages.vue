<template>
  ...
  >
    ...
    <v-list>
      ...
      >
        <v-list-item-content>
          <v-list-item-title>Zählstelle {{ zaehlstelle.nummer }}</v-list-item-title>
          <v-list-item-subtitle
              v-for="(zaehlung, index) in getZaehlungenWithUnreadMessagesFromZaehlstelle(zaehlstelle)"
              :key="index">
            {{ $d(Date.parse(zaehlung.datum), 'short', 'de-DE') }} {{ zaehlung.projektName }}
          </v-list-item-subtitle>
        </v-list-item-content>
      </v-list-item>
    </v-list>
...
export default class UnreadMessages extends Vue {
  ...
  private loadZaehlstellenWithUnreadMessages(): void {
    ZaehlstellenService.getZaehlstellenByUnreadMessages(UnreadMessages.PARTICIPANT_ID_MOBILITAETSREFERAT)
        .then(result => {
   ...
   * @private
   */
  private getZaehlungenWithUnreadMessagesFromZaehlstelle(zaehlstelleDto: ZaehlstelleDTO): ZaehlungDTO[] {
